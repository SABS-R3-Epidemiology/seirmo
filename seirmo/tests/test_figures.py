...
class TestSubplotFigure(unittest.TestCase):
    ...
    def test_add_data(self):
        ...
        # Test the added data in the IncidenceNumberPlot
        # is as expected
        ...
        # is the same as what we give
        np.testing.assert_array_equal(
            test_plot._incidence_num_plot._fig['data'][0]['y'],
            np.array([1, 2, 3, 4, 5, 6, 7]))
    ...
    def test_add_simulation(self):
        ...
        # Test the added simulation data in the IncidenceNumberPlot
        # is as expected
        # Test the times in the IncidenceNumberPlot
        # is the same as what we give
        np.testing.assert_array_equal(
        ...
        # Test the times in the CompartmentPlot is the same as what we give
        np.testing.assert_array_equal(
            test_plot._compartment_plot._fig['data'][0]['x'],
            np.array([0, 1, 2, 3, 4, 5, 6]))
        np.testing.assert_array_equal(
            test_plot._compartment_plot._fig['data'][1]['x'],
            np.array([0, 1, 2, 3, 4, 5, 6]))
        np.testing.assert_array_equal(
            test_plot._compartment_plot._fig['data'][2]['x'],
            np.array([0, 1, 2, 3, 4, 5, 6]))
        np.testing.assert_array_equal(
            test_plot._compartment_plot._fig['data'][3]['x'],
            np.array([0, 1, 2, 3, 4, 5, 6]))

        # Test the S, E, I, R in the CompartmentPlot
        # is the same as what we give
        np.testing.assert_array_equal(
            test_plot._compartment_plot._fig['data'][0]['y'],
            np.array([1, 2, 3, 4, 5, 6, 7]))
        np.testing.assert_array_equal(
            test_plot._compartment_plot._fig['data'][1]['y'],
            np.array([2, 3, 4, 5, 6, 7, 8]))
        np.testing.assert_array_equal(
            test_plot._compartment_plot._fig['data'][2]['y'],
            np.array([3, 4, 5, 6, 7, 8, 9]))
        np.testing.assert_array_equal(
            test_plot._compartment_plot._fig['data'][3]['y'],
            np.array([4, 5, 6, 7, 8, 9, 10]))

    def test_get_subplots(self):

        # Add simulation for the subplots
        test_plot = plots.SubplotFigure()
        data = pd.DataFrame({
            'Time': [0, 1, 2, 3, 4, 5, 6],
            'Incidence Number': [1, 2, 3, 4, 5, 6, 7],
            ...
            'Recovered': [4, 5, 6, 7, 8, 9, 10]})
        test_plot.add_simulation(data)
        test_plot.get_subplots()

        # Test the layouts of the subplots are as expected
        # Test the layout for the for IncidenceNumberPlot
        np.testing.assert_equal(
            test_plot._fig['layout']['xaxis']['title']['text'],
            'Time'
        )
        ...
        # Test the layout for the for CompartmentPlot
        np.testing.assert_equal(
            test_plot._fig['layout']['xaxis2']['title']['text'],
            'Time'
        )
