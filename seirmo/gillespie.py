...
def solve_gillespie(propensities: typing.Callable[[np.ndarray],np.ndarray], initial_cond : np.ndarray, t_span : []):
    """ Solve an initial_cond value problem using gillespie algorithm

    This function numerically integrates a system of ordinary differential
    equations given initial_cond conditions. We use the Gillespie algorithm with tau-leaping,
    so that the time step is not uniform but sampled randomly.

    Parameters
    ----------
    propensities : callable of form propensities(state)
        State is an array of length (N+1) of time and counts in each compartment (for N compartments)
        Will return an NxN array in the form of a transition matrix for propensities
    t_span : 2-list of floats
        Internal of integration (t_start, t_end). The solver starts at t_start and will finish
        once the first random time_step exceeds t_end (Note that the final value may be greater than t_end)
    ...
    Numpy array of form [Time, Compartment_1, ...], ie [time, S, E, I, R]
        Note that time steps are random (not uniform), and final timestep will lie 
        after the end of the time_span given

    """
    if len(t_span) != 2:
        raise ValueError("`t_span must be 2-dimensional - of form [start, end]")
